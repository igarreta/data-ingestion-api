...
| `timestamp` | string (ISO-8601) | no | Pump start time |
...
curl -X POST http://192.168.1.3:8000/homeassistant/events \
  ...
  -d '{"entity_id":"sensor.bomba_agua_run_secs","value":312,"timestamp": "2026-05-31T12:30:30"}'
```
...
rest_command:
  bomba_agua_run_secs:
    ...
    headers:
      ...
      Content-Type: application/json
    payload: >
      {"entity_id":"sensor.bomba_agua_run_secs","value":{{ value }},"timestamp": "2026-05-31T12:30:30"}
